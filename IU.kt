...
/**
 * Clase que contendra todo el programa,El juego consiste en darle al 'START', que empieza la cuenta atrás. Se van mostrando frases, que pueden ser verdaderas o falsas,
   segun vayan apareciendo el jugador le dará al botón de verdadero o falso
   y si coincide le suma uno a su puntuación, que se muestra en el cuadro de texto puntuación.
 *
...
// para crear los objetos de las frases
    ...
    /**
     * Boton que se usa para aumentar en 5 el tiempo AÑADIDO PARA PRACTICAR
     */

    @Composable
    fun botonAumentarTiempo() {
        Button(
            onClick = {
                // Incrementar el tiempo en 5 segundos
                tiempoActual.value += 5
            },
            modifier = Modifier
                .height(35.dp)
                .width((350 / 2).dp)
    ...
    fun botonVerdadero() {
        Button(
            onClick = {
                // si la frase es verdadera y el tiempo es mayor a 0
                if (fraseActual.value.verdadero and (tiempoActual.value > 0) ) {
    ...
    fun botonFalso() {
        Button(
            onClick = {
                // si la frase es falsa y el tiempo es mayor a 0
                if (!fraseActual.value.verdadero and (tiempoActual.value > 0)) {
                    // se suma uno a la puntuacion
                    puntuacion.value++
                    // se cambia la frase
    ...
    fun cuadroTextoPuntuacion() {
        ...
        // SI DA PROBLEMAS EL TOSTRING en una lista se pone toList()
    ...
    @Composable
    fun juego(){
        // meto los valores en aux
        aux()
        // Column = caja vertical
        Column{
            // Nueva fila para la anterior columna
            Row {
                // Le añadimos boton
                botonStart()
                // Le añadimos un cuadro de texto que implementa una funcion
